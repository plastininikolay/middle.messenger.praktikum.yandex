...
import {AuthPage} from "./pages/AuthPage/AuthPage.ts";
import {ProfileEditPage} from "./pages/ProfileEditPage/ProfileEditPage.ts";
...
import {ErrorPage} from "./pages/ErrorPage/ErrorPage.ts";
import {ChangePasswordPage} from "./pages/ChangePassword/ChangePassword.ts";
import {mockChatsData, mockProfileData} from "./mocks.ts";
...
export default class App {
	...
	private getCurrentPage(): PAGE_NAMES {
		...
		return Object.values(PAGE_NAMES).includes(page) ? page : PAGE_NAMES.notFound;
	}

	render(): string {
		const PageClass = this.pagesMap[this.state.currentPage] || this.pagesMap[PAGE_NAMES.notFound];
