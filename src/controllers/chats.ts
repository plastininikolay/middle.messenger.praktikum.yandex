...
import {ChatsRequest, CreateChatRequest, DeleteChatRequest} from "../api/types";
...
class ChatsController {
	...
	public async deleteChat(data: DeleteChatRequest) {
		try {
			store.set("requestStatus.loading", true);
			store.set("requestStatus.error", null);
			const chatResponse = await chatsAPI.deleteChat(data);
			if (chatResponse?.reason) {
				throw new Error(chatResponse.reason);
			}
			store.set("requestStatus.loading", false);
			...
			if (error instanceof Error) {
				...
				store.set("requestStatus.error", "Произошла ошибка при удалении");
			}
		} finally {
			store.set("requestStatus.loading", false);
