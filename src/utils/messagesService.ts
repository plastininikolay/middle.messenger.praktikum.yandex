...
import {BASE_URL, BASE_WEBSOCKET_URL} from "../constanst.ts";
...
class MessagesService {
	...
	async connect(chatId: number, userId: number) {
		...
		this.close();
		try {
			const response = await this.http.post(`${BASE_URL}/chats/token/${chatId}`);
			let data;
			try {
			...
			this.socket = new WebSocket(`${BASE_WEBSOCKET_URL}/chats/${userId}/${chatId}/${token}`);
